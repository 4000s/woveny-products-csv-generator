...
class WovenyProductGeneratorService(
    ...
    fun generateForRugs(generateProductRequest: GenerateProductRequest): Int {
        ...
        val extension = ".csv"
        val csvPrinter = csvPrinter(fileName + extension, CSV_HEADERS_FOR_RUG)
        ...
        for (index in spreadSheetDocument.rows.indices) {
            try {
                ...
                println("ProductGenerator, some fields missing for SKU: ${generateModel(index, spreadSheetDocument)}")
            }
        }
        csvPrinter.flush()
    ...
    fun generateForPillows(generateProductRequest: GenerateProductRequest): Int {
        val spreadSheetDocument =
            googleDocsService.getPillowsDocument(generateProductRequest.startIndex, generateProductRequest.endIndex)

        val (startSku, endSku) = getStartEndSku(spreadSheetDocument)
        val fileName = "${CURRENT_DATE.format(FORMAT)}-Products($startSku-$endSku)"
        val extension = ".csv"
        val csvPrinter = csvPrinter(fileName + extension, CSV_HEADERS_FOR_PILLOW)
    ...
    private fun csvPrinter(fileName: String, headers: Array<String>): CSVPrinter {
        val out =
            FileWriter(fileName)
